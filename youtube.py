...
st.image("https://cdn.pixabay.com/photo/2019/12/18/04/14/youtube-4702984_1280.png", width=600)
st.logo(image="https://cdn.pixabay.com/photo/2023/01/23/05/21/youtube-7737850_1280.png",size="large",link="https://www.youtube.com/",icon_image="https://www.freeiconspng.com/uploads/youtube-logo-drawn-7.png")

selected=option_menu(
    menu_title="☺️Menu",
    options=["Description", "Data collection and migration","Table visualization","Analytics"],
    icons=['file-earmark-font','database-check','table','file-bar-graph'],
    menu_icon='cast',
    default_index=0,
...
if selected == "Description":
    st.header("About")
    st.markdown("_User-friendly Streamlit application that utilizes the [Google API](https://developers.google.com/explorer-help) to extract information on a YouTube channel, stores it in a SQL database, and enables users to search for channel details and join tables to view data here._")
    st.subheader("Skillsets and Functions")
    st.caption("Skill sets")
    col=list([["Python scripting", "API integration","Data Collection", "Streamlit and Pandas", "Data Management using SQL"]])
    st.write_stream(col)
    st.caption("Functions")
    cols=list([["Analytics between channels", "Check likes", "Check views", "Check comments", "check average time duration of videos"]])
    st.write_stream(cols)
    st.caption("Note:")
    st.markdown("_You can utilize this app by giving 'Channel id' as an input from any channel in the youtube , which can be obtained by clicking the Logo at the Top-left corner of the page._")
    
if selected == "Data collection and migration":    
    ...
    if st.button("Collect and store data"):
        ...
        try:
            if channel_id in ch_ids:
                st.warning("Channel ID exist already")
                
            else:
                insert=Channel_details(channel_id)
                st.success(insert)
                st.balloons()
                st.write(ch_data['channel_details']['channel_name'])
        except:
    ...
    if st.button("CLick here for tabularization"):
        Table=tables(unique_channel)
        st.success(Table)
        st.balloons()

if selected == "Table visualization":       
...
if selected == "Analytics":  
    ...
    if Question == "1.The names of all the videos and their corresponding channels":
            ...
            TB=st.checkbox("Table")
            if TB:
                st.write(df2)
            CB=st.checkbox("Chart")
            if CB:
                st.bar_chart(data=df2,x="Channel name",y="Total videos")
            ...
            CB=st.checkbox("Chart")
            if CB:
                st.bar_chart(data=df3,x="Video_Name",y="Views")
            ...
            CB=st.checkbox("Chart")
            if CB:
                st.bar_chart(data=df4,x="Video title",y="Total comments")
            ...
            CB=st.checkbox("Chart")
            if CB:
                st.bar_chart(data=df5,x="Channel name",y="likes count")
            ...
            CB=st.checkbox("Chart")
            if CB:
                st.bar_chart(data=df6,x="Video title",y="likes count",color='#8B0000',use_container_width= True)
            ...
            CB=st.checkbox("Chart")
            if CB:
                st.bar_chart(data=df7,x="Channel name",y="Views count",color='#CD5C5C',use_container_width= True)
            ...
            CB=st.checkbox("Chart")
            if CB:
                st.bar_chart(data=df1,x="Channeltitle",y="AVG_duration",color="#00FFFF")
            
    elif Question == "10. Top 10 Videos with highest number of comments":
            ...
            # conn.commit()
            t10=cursor.fetchall()
            df10=pd.DataFrame(t10,columns=["Video title","Channel name","Total comments"]).sort_values(by="Total comments",ascending=False).head(10)
            TB=st.checkbox("Table")
            if TB:
                st.write(df10)
            CB=st.checkbox("Chart")
            if CB:
                st.bar_chart(data=df10,x="Video title",y="Total comments",color="#F08080")